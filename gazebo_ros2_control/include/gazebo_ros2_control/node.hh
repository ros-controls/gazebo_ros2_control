...
public:
      template < typename M >
      transport::PublisherPtr Advertise(
        const std::string & _topic,
...
public:
      void Publish(
        ...
        const google::protobuf::Message & _message)
      {
        transport::PublisherPtr pub = this->Advertise(
          _topic,
...
private:
      std::vector < PublisherPtr > ::iterator publishersIter;
...
private:
      Callback_M callbacks;

private:
      std::map < std::string, std::list < std::string >> incomingMsgs;

      /// \brief List of newly arrive messages

private:
      std::map < std::string, std::list < MessagePtr >> incomingMsgsLocal;
...
private:
      boost::mutex publisherMutex;

private:
      boost::mutex publisherDeleteMutex;
...
private:
      boost::recursive_mutex processIncomingMutex;
